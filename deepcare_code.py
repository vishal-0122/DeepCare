...
# Disable whole-stage codegen for Colab
...
# Store results: (Model, Partitions, AUC, Accuracy, Time)
results = []

# Partition configurations to simulate 1 to 4 worker nodes
partitions = [1, 2, 3, 4]

# 1. Logistic Regression
for p in partitions:
    print(f"\n Logistic Regression | Partitions: {p}")
    train_p = train.repartition(p)
    start = time.time()
    lr = LogisticRegression(featuresCol="features", labelCol="drop_off")
    lr_model = lr.fit(train_p)
...
# Final Output
print("\n Model Evaluation at Different Partition Levels:")
print(f"{'Model':<25} {'Partitions':<10} {'AUC':<8} {'Accuracy':<10} {'Time (s)':<10}")
for name, part, auc, acc, duration in results:
    print(f"{name:<25} {part:<10} {auc:.4f}   {acc:.4f}     {duration}s")
